<script lang="ts">
	...
	import type { ChannelMessage, MessageFileTransfer } from '$lib/types/types';
  ...
  function update_file(m: MessageFileTransfer) {
    for (let i = 0; i < messages.length; i++) {
      ...
      if (msg.type === p2p.MESSAGE_TYPE.FILE_TRANSFER && msg.id === m.id) {
        messages[i] = m;
        // console.log(1, msg);
        // const new_msg = cb(msg as MessageFileTransfer);
        // console.log(2, new_msg);
        // messages[i] = new_msg;
        break;
      }
    }
...
<main class="mx-auto max-w-3xl p-4 font-sans">
  <div class="flex">
	...
	<div class="flex h-96 flex-col overflow-clip rounded-lg border border-gray-300 md:h-[500px]">
		<Messages {messages} {isHost} {cancel_file_transfer} {download_file} />
